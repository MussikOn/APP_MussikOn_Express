...
export const RegisterDTO = Joi.object({
  ...
  roll: Joi.string().valid('admin', 'superadmin', 'eventCreator', 'musician').default('eventCreator').messages({
...
export const CreateEventDTO = Joi.object({
  ...
  eventType: Joi.string().valid(
    'concierto', 'boda', 'culto', 'evento_corporativo', 
    'festival', 'fiesta_privada', 'graduacion', 'cumpleanos', 'otro'
  ).required().messages({
  ...
  instrument: Joi.string().valid(
    'guitarra', 'piano', 'bajo', 'bateria', 'saxofon', 
    'trompeta', 'violin', 'canto', 'teclado', 'flauta', 'otro'
  ).required().messages({
  ...
  recommendations: Joi.array().items(Joi.string().max(200)).max(10).optional().messages({
...
export const UploadImageDTO = Joi.object({
  category: Joi.string().valid('profile', 'post', 'event', 'gallery', 'admin').required().messages({
...
export const CreateMessageDTO = Joi.object({
  ...
  type: Joi.string().valid('text', 'image', 'audio', 'file').default('text').messages({
...
export const UpdateUserDTO = Joi.object({
  ...
  roll: Joi.string().valid('admin', 'superadmin', 'eventCreator', 'musician').optional().messages({
    'any.only': 'Rol debe ser uno de: admin, superadmin, eventCreator, musician'
  })
});
...
export const PaginationDTO = Joi.object({
  ...
  sortBy: Joi.string().valid('createdAt', 'updatedAt', 'name', 'date').default('createdAt').messages({
...
export const EventFiltersDTO = PaginationDTO.keys({
  status: Joi.string().valid(
    'pending_musician', 'musician_assigned', 'completed', 'cancelled', 'musician_cancelled'
  ).optional().messages({
