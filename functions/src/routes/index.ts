// Exportar todas las rutas
export { default as authRoutes } from './authRutes';
export { default as adminRoutes } from './adminRoutes';
export { default as superAdminRoutes } from './superAdminRouter';
export { default as imagesRoutes } from './imagesRoutes';
export { default as musicianProfileRoutes } from './musicianProfileRoutes';
export { default as eventsRoutes } from './eventsRoutes';
export { default as musicianRequestRoutes } from './musicianRequestRoutes';
export { default as chatRoutes } from './chatRoutes';
export { default as advancedSearchRoutes } from './advancedSearchRoutes';
export { default as optimizationRoutes } from './optimizationRoutes';
export { default as paymentSystemRoutes } from './paymentSystemRoutes';
export { default as idriveHealthRoutes } from './idriveHealthRoutes'; 