...
export class ChatService {
   ...
   */
  async createConversation(participants: string[], isGroup: boolean = false, groupName?: string, groupAdmin?: string): Promise<Conversation> {
    try {
   ...
   * Enviar un mensaje
   */
  async sendMessage(conversationId: string, senderId: string, content: string, type: 'text' | 'image' | 'audio' | 'file' = 'text', metadata?: any): Promise<Message> {
    try {
      ...
      const message: Message = {
        id: db.collection('conversations').doc(conversationId).collection('messages').doc().id,
        conversationId,
        senderId,
        senderName,
        content,
  ...
  async searchConversations(userId: string, searchTerm: string): Promise<Conversation[]> {
    try {
      ...
      const conversations = await this.getUserConversations({ userId, limit: 100 });
      ...
      return conversations.filter(conversation => {
        ...
        const groupMatch = conversation.groupName?.toLowerCase().includes(searchTerm.toLowerCase());

        // Buscar en último mensaje
        const messageMatch = conversation.lastMessage?.content.toLowerCase().includes(searchTerm.toLowerCase());
   ...
   * Buscar mensajes
   */
  async searchMessages(conversationId: string, searchTerm: string, limit: number = 20): Promise<Message[]> {
    try {
      const query = db.collection('conversations')
        .doc(conversationId)
        .collection('messages')
   ...
   */
  async addParticipantToGroup(conversationId: string, participantId: string, adminId: string): Promise<void> {
    try {
   ...
   */
  async removeParticipantFromGroup(conversationId: string, participantId: string, adminId: string): Promise<void> {
    try {
      const conversation = await this.getConversationById(conversationId);
      if (!conversation) {
        throw new Error('Conversación no encontrada');
  ...
  async getChatStats(userId: string): Promise<{
    ...
    try {
      ...
      for (const conversation of conversations) {
        const messageCount = await this.getConversationMessages(conversation.id, 1000);
