...
export class IDriveHealthService {
  ...
  async checkHealth(): Promise<IDriveHealthStatus> {
    ...
    try {
      logger.info('[src/services/idriveHealthService.ts] Iniciando verificación de salud de IDrive E2');
      ...
      logger.info('[src/services/idriveHealthService.ts] Verificación de salud completada', {
        metadata: {
          isHealthy,
          responseTime,
      ...
      logger.error('[src/services/idriveHealthService.ts] Error en verificación de salud:', error instanceof Error ? error : new Error(String(error)));
      return this.healthStatus;
    }
  }
  ...
  private async testBucketAccess(): Promise<boolean> {
    try {
      ...
      logger.error('[src/services/idriveHealthService.ts] Error verificando acceso al bucket:', error instanceof Error ? error : new Error(String(error)));
      return false;
    }
  }
  ...
  private async testUpload(): Promise<boolean> {
    try {
      ...
      logger.error('[src/services/idriveHealthService.ts] Error en prueba de subida:', error instanceof Error ? error : new Error(String(error)));
      return false;
    }
  }
  ...
  private async testDownload(): Promise<boolean> {
    try {
      ...
      logger.error('[src/services/idriveHealthService.ts] Error en prueba de descarga:', error instanceof Error ? error : new Error(String(error)));
      return false;
    }
  }
