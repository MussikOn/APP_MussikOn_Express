...
export const registerDTO = Joi.object({
  ...
  role: Joi.string().valid('user', 'musician', 'admin', 'super_admin').default('user'),
...
export const updateMusicianRequestDTO = Joi.object({
  status: Joi.string().valid('pending', 'accepted', 'rejected', 'canceled').required(),
...
export const searchUsersDTO = Joi.object({
  query: Joi.string().min(1).max(100).optional(),
  role: Joi.string().valid('user', 'musician', 'admin', 'super_admin').optional(),
  location: Joi.object({
    latitude: Joi.number().min(-90).max(90).required(),
    longitude: Joi.number().min(-180).max(180).required(),
    radius: Joi.number().min(1).max(100).default(10)
...
export const optimizeRouteDTO = Joi.object({
  ...
  avoid: Joi.array().items(Joi.string().valid('tolls', 'highways', 'ferries')).optional()
});
...
export const validatePaymentMethodDTO = Joi.object({
  ...
  expiryYear: Joi.number().min(new Date().getFullYear()).max(new Date().getFullYear() + 20).required(),
